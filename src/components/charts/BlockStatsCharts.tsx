...
export function BlockStatsCharts() {
  const { stats, loading } = useBlockStats();
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-black p-6 rounded-xl shadow-xl border border-green-500/20">
        <h3 className="text-lg font-semibold text-green-400 mb-4">Block Time</h3>
      ...
      <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-black p-6 rounded-xl shadow-xl border border-green-500/20">
        <h3 className="text-lg font-semibold text-green-400 mb-4">Transactions per Block</h3>
        <div className="h-64 bg-gradient-to-r from-gray-800/50 to-gray-900/50 rounded-lg p-4">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={stats.transactionsHistory}>
              <XAxis 
                dataKey="timestamp" 
                stroke="#4ade80"
                tickFormatter={(value) => new Date(value * 1000).toLocaleTimeString()}
